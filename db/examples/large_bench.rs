...
fn main() {
    println!("=== LARGE Scale Index Performance Benchmark ===\n");
    
    // Clean up any existing test db
    let _ = std::fs::remove_dir_all("large_bench_db");
    
    let db = KeyLite::open("large_bench_db").unwrap();
    ...
    // Benchmark: Insert 10,000 documents
    let n = 10_000;
    ...
    for i in 0..n {
        let doc = serde_json::json!({
            "name": format!("User{}", i % 100),  // 100 unique names, 100 docs per name
        ...
        if (i + 1) % 1000 == 0 {
            println!("  {} documents inserted...", i + 1);
        }
    }
    ...
    // Benchmark: get_by_index on non-unique index (returns 100 docs)
    ...
    println!("Benchmark 3: Query by unique index (email = 'user5000@test.com')");
    let start = Instant::now();
    let results = db.get_by_index("users", "email", "user5000@test.com".into()).unwrap();
    let query_time = start.elapsed();
    println!("  Found {} documents", results.len());
    println!("  Query time: {:?}\n", query_time);
    ...
    println!("Benchmark 5: 1000 random queries by index");
    let start = Instant::now();
    for i in 0..1000 {
    ...
    println!("  Per query: {:?}", multi_query_time / 1000);
    println!("  Throughput: {:.0} queries/sec\n", 1000.0 / multi_query_time.as_secs_f64());
